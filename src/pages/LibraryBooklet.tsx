    ...
    Promise.all([getImportedLibraryPack(bookletId), getLibraryReadingProgress(bookletId)])
      .then(async ([data, savedProgress]) => {
        ...
        if (data) {
          ...
          if (check.allowed) logActivity("library_booklet_opened", { targetType: "libraryBooklet", targetId: bookletId, metadata: { title: data.manifest.title } }).catch(() => undefined);
        }
      })
  ...
  return (
    <section className="page grid gap-4">
      ...
      <section className="panel grid gap-3 p-4">
        ...
        <p className="text-sm leading-6 text-paper/65">{pack.content.booklet.author} / {pack.manifest.category} / version {pack.manifest.version}</p>
        {progress && <p className="border-l-2 border-signal pl-3 text-sm leading-6 text-paper/70">Saved {new Date(progress.updatedAt).toLocaleString()}.</p>}
      </section>
          ...
          <div key={chapter.id} className="p-4">
            ...
            <div className="mt-3 grid gap-2">
              {chapter.sections.map((section) => (
                <Link key={section.id} className="border border-white/10 bg-black/20 p-3 hover:bg-white/5" to={`/library/${bookletId}/chapter/${chapter.id}/section/${section.id}`}>
                  <span className="text-xs font-bold uppercase tracking-[0.14em] text-paper/45">Section {section.sectionNumber}</span>
                  <span className="ml-2 font-bold">{section.heading || "Untitled section"}</span>
                </Link>
              ))}
            </div>
          </div>
